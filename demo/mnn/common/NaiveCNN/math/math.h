...
inline float sqrt_newton(float x) {
    ...
    } while (BSJ_ABS(result - last_value) > EN);
    return result;
}
...
inline float atan(float x) {
    return x * (0.78539816 - (BSJ_ABS(x) - 1) * (0.2447 + 0.0663 * DL_ABS(x)));
    // float s = x*x;
...
inline float atan2(float x, float y) {
    ...
    float ay = BSJ_ABS(y);
    float eps = 1e-8;
    float a = BSJ_MIN(ax, ay) / (BSJ_MAX(ax, ay) + eps);
