...
from yolox.data.data_augment import preproc_1 as preprocess
...
def make_parser():
    ...
    parser.add_argument(
        "--save_result",
        action="store_true",
        help="whether to save the inference result of image to an external file or not.",
    )
    parser.add_argument(
        "--save_pr_curve",
        action="store_true",
        help="whether to save the pr curve of image to an external file or not.",
    )
    parser.add_argument(
        "--save_labelimg_txt",
        action="store_true",
        help="whether to save the labelimg txt of image to an external file or not.",
    )
    return parser
...
if __name__ == '__main__':
    ...
    for file_num, imgpath in enumerate(images_path):
        print(f"Processing {file_num} th image: {imgpath}")
        imgpath = str(imgpath)
        origin_img = cv2.imread(imgpath)
        resize_img = cv2.resize(origin_img, (input_shape[1], input_shape[0]))
        img = preprocess(resize_img)
        ...
        w_ratio = origin_img.shape[1] / input_shape[1]
        h_ratio = origin_img.shape[0] / input_shape[0]

        boxes_xyxy[:, 0] *= w_ratio
        boxes_xyxy[:, 1] *= h_ratio
        boxes_xyxy[:, 2] *= w_ratio
        boxes_xyxy[:, 3] *= h_ratio
        ...
        if args.save_result:
            ...
            if dets is not None:
                ...
                if save_img:
                    save_path="/code/YOLOX/demo/ONNXRuntime/onnx_%d.jpg"%file_num
