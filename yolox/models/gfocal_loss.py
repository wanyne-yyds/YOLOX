...
from ..utils.utils import weighted_loss, weighted_Floss
...
@weighted_Floss
def focal_loss(loss_fcn, pred, target, gamma=2, alpha=0.25):
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2, alpha=0.25, reduction="mean", loss_weight=1.0):
    ...
    def forward(
        ...
        assert reduction_override in (None, "none", "mean", "sum")
        reduction = reduction_override if reduction_override else self.reduction
        loss = self.loss_weight * focal_loss(
            self.loss_fcn,
            pred,
            target,
            weight=weight,
