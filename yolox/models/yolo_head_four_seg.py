...
from .network_blocks import BaseConv, DWConv, BottleneckCSP
from .network_blocks import Conv as Conv_seg

class YOLOXHeadFour_Seg(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
        ...
        for i in range(len(in_channels)):
            ...
            if i == 0:
                self.seg_preds.append(
                    nn.Sequential(
                        *[
                            Conv_seg(c1=int(in_channels[i] * width), c2=128, k=3, s=1),
                            Upsample(scale_factor=2, mode="nearest"),
                            BottleneckCSP(128, 64, 1, False),
                            Conv_seg(c1=64, c2=32, k=3, s=1),
                            Upsample(scale_factor=2, mode="nearest"),
                            Conv_seg(c1=32, c2=16, k=3, s=1),
                            BottleneckCSP(16, 8, 1, False),
                            Upsample(scale_factor=2, mode="nearest"),
                            Conv_seg(c1=8, c2=2, k=3, s=1),
                        ]
                    )
                )
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        if self.training:
            ...
            if not self.conv_models_deploy:
                self.hw = [x.shape[-2:] for x in outputs[1:]]
                # [batch, n_anchors_all, 85]
                outputs_d = torch.cat(
                    [x.flatten(start_dim=2) for x in outputs[1:]], dim=2
                ).permute(0, 2, 1)
