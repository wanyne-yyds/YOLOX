...
from .network_blocks import GhostBottleneck
from .network_blocks import ConvModule, DepthwiseConvModule
...
class YOLOGhostPAN(nn.Module):
    ...
    def __init__(
        ...
        super(YOLOGhostPAN, self).__init__()
        assert num_extra_level >= 0
        assert num_blocks >= 1
        if backbone == "CSPDarknet":
            self.backbone = CSPDarknet(depth, width, depthwise=depthwise, act=act)
        elif backbone == "MobileNetV2":
            self.backbone = MobileNetV2(widen_factor=width, inverted_residual_setting=mobilenet_invertedt, out_indices=out_indices,pretrained=False,activation=act)
        self.in_features = in_features
        self.in_channels = in_channels
        out_channels = 16
        conv = DepthwiseConvModule if depthwise else ConvModule
    ...
    def forward(self, inputs):
        ...
        out_features = [
            reduce(input_x) for input_x, reduce in zip(out_features, self.reduce_layers)
        ]


        # top-down path
        inner_outs = [out_features[-1]]
        for idx in range(len(self.in_channels) - 1, 0, -1):
            feat_heigh = inner_outs[0]
            feat_low = out_features[idx - 1]
        ...
        for extra_in_layer, extra_out_layer in zip(
            ...
            outs.append(extra_in_layer(out_features[-1]) + extra_out_layer(outs[-1]))
        return tuple(outs)