...
from yolox.data.data_augment import preproc_centered
...
class CommonDualTransform(A.DualTransform):
...
class RandomResizedCropIfEmpty(A.RandomResizedCrop):
...
class MyRandomSizedBBoxSafeCrop(A.RandomResizedCrop):
...
class DetectedBBoxSafeCrop(MyRandomSizedBBoxSafeCrop):
    ...
    def get_crop_bboxes(self, params):
        ...
        if self.detector is not None:
            ...
            bbox_num = len(bboxes)
            if bbox_num > 0:
                bboxes = A.convert_bboxes_to_albumentations(bboxes, 'pascal_voc', img_h, img_w)
...
class RandomCropAndPaste(A.DualTransform):
...
class MyGridDropout(A.GridDropout):
    ...
    def apply(self, image, holes=(), **params):
        if len(self.bboxes) > 0:
            ...
            img_orig = image.copy()
            img_cutout = A.augmentations.dropout.functional.cutout(image, holes, self.fill_value)
...
class RandomCopyFpBbox(RandomCopyBbox):
    ...
    def __init__(self, detector, scale=(0.8, 1.5), ratio=None, interpolation=cv2.INTER_LINEAR, bbox_min_scale=None,
        ...
        self.rotate_limit = A.to_tuple(rotate_limit)
...
class ToColor(A.ImageOnlyTransform):
...
imgpath = r"/code/data/s_BSD/hyh_bsd_yoloformat/train_640_0.004_221026/mixed/Normal_0.004/front_view/2021-07-14_imgs_out_zlj/images/115015676000679_210629103255_103_2_wjc_20210630_wjc_20210701.jpg"
img_size = [320, 576]
swap = (1, 2, 0)
...
transform = A.Compose([
    ...
    A.PadIfNeeded(min_height=320, min_width=576, border_mode=0, p=1),
    RandomCopyBbox(p=1, bbox_min_scale=0.008, scale=(0.7, 1.5)),
    A.CropAndPad(percent=(-0.1, 0.1), keep_size=True, sample_independently=True, p=0.5),
    ...
    A.OneOf([
        MyRandomSizedBBoxSafeCrop(height=320, width=576, erosion_rate=0.0, scale=(
            0.1, 1), ratio=(1.4, 2.2), p=0.3, alignment="random"),
        MyRandomSizedBBoxSafeCrop(height=320, width=576, erosion_rate=0.0, scale=(
            0.1, 1), ratio=(1.7, 1.9), p=0.3, alignment="random"),
    ...
    A.OneOf([
        ...
        # DetectedBBoxSafeCrop(height=320, width=576, crop_bbox_num=1, scale=(
        #     0.05, 1), ratio=(1.0, 2.5), p=0.3, alignment="random", score_weight_power=0.75),
        # RandomCopyFpBbox(p=0.4, bbox_min_scale=0.004, scale=(0.6, 2.5), apply_times=2, copy_bbox_num=2, bbox_zoom_scale=(-0.05, 0.3),
        #                  rotate_limit=30),
    ], p=0.3),
    RandomEraseBbox(scale=(0.6, 0.75), p=0.2),
    A.OneOf([
        A.Resize(p=0.3, height=320, width=576, interpolation=cv2.INTER_NEAREST),
        A.Resize(p=0.4, height=320, width=576, interpolation=cv2.INTER_LINEAR),
        A.Resize(p=0.1, height=320, width=576, interpolation=cv2.INTER_CUBIC),
        A.Resize(p=0.05, height=320, width=576, interpolation=cv2.INTER_LANCZOS4),
        A.Resize(p=0.05, height=320, width=576, interpolation=cv2.INTER_AREA),
    ], p=1),
    A.ToGray(p=0.1),
    A.OneOf([
        A.RandomBrightnessContrast(p=0.1, brightness_limit=(-0.3, 0), contrast_limit=(-0.3, 0), brightness_by_max=False),
        A.ColorJitter(p=0.2, brightness=0.3, contrast=0.5, saturation=0, hue=0),
        A.ColorJitter(p=0.2, brightness=0.3, contrast=0.5, saturation=0.1, hue=0.1),
    ], p=0.4),
    A.OneOf([
        A.Downscale(scale_min=0.8, scale_max=0.97, p=0.2),
        A.JpegCompression(quality_lower=40, quality_upper=95, p=0.8),
        A.GaussNoise(var_limit=(10.0, 50.0), p=0.2),
    ], p=0.02),
    A.OneOf([
        A.MotionBlur(blur_limit=(3, 5), p=0.6),
        A.GaussianBlur(blur_limit=(3, 5), sigma_limit=3, p=0.8),
        A.MedianBlur(blur_limit=(3, 5), p=0.2),
        A.Blur(blur_limit=(3, 5), p=0.2),
    ], p=0.05),
    A.HorizontalFlip(p=0.5),
    # ToColor(color_format="bgr", p=1),
],
    bbox_params=A.BboxParams(format='yolo', min_area=20, min_visibility=0.1, label_fields=['class_labels'],))

bboxes = [
[0.983594,0.470833,0.032813,0.200000],
[0.704688,0.506250,0.075000,0.266667],
[0.767188,0.522917,0.093750,0.316667],
]
class_labels = ['person','person','person']
...
while True:
    ...
    for i in range(len(bboxes)):
        x, y, w, h = bboxes[i]
        x1, y1, x2, y2 = yolo2voc(x, y, w, h, width, height)
        cv2.rectangle(transformed_image, (int(x1), int(y1)), (int(x2), int(y2)), color=[0, 0, 255], thickness=2)
